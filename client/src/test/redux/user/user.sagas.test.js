...
describe("on signup success saga", () => {
  it("should trigger on EMAIL_SIGN_UP_SUCCESS", () => {
    const generator = onEmailSignUpSuccess();
    expect(generator.next().value).toEqual(
      takeLatest(UserActionTypes.EMAIL_SIGN_UP_SUCCESS, signInAfterSignUp)
    );
  });
});

describe("on signup start saga", () => {
  it("should trigger on EMAIL_SIGN_UP_START", () => {
    const generator = onEmailSignUpStart();
    expect(generator.next().value).toEqual(
      takeLatest(UserActionTypes.EMAIL_SIGN_UP_START, signUpWithEmail)
    );
  });
});
...
describe("on check user session saga", () => {
  it("should trigger on CHECK_USER_SESSION", () => {
    ...
    expect(generator.next().value).toEqual(
      takeLatest(UserActionTypes.CHECK_USER_SESSION, getUserSession)
    );
  });
});
...
describe("on sign up saga", () => {
  ...
  const generator = signUpWithEmail(mockAction);
...
describe("is user authenticated saga", () => {
  const generator = getUserSession();
  ...
  it("should call signInFailure on error", () => {
    const newGenerator = getUserSession();
    newGenerator.next();
    expect(newGenerator.throw("error").value).toEqual(
